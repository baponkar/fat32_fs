...
static FSInfo *create_fsinfo_sector() {
...
static bool initialize_fat_tables(uint64_t fat_start, uint32_t fat_sector_size) {
    ...
    if(!disk_write(fat_start, 1, fat_buffer)){   // Writing first three FAT entries
        return false;
    }
    ...
    while (sectors_written < fat_sector_size) {
        ...
        if (!disk_write( fat_start + sectors_written, sectors_to_write, zeros)) {
            free(zeros);
            return false;
        }
...
static bool fat32_zero_cluster(uint32_t cluster_no)
{
