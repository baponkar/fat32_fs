...
void file_test_func()
{
    ...
    /* create file */
    if (!f_open(&file, "test.txt", FA_CREATE_ALWAYS | FA_WRITE))
    {
    ...
    /* reopen for read */
    if (!f_open(&file, "test.txt", FA_READ))
    {
    ...
    /* truncate test */
    if (f_open(&file, "test.txt", FA_WRITE))
    {
        f_lseek(&file, 5);
        f_truncate(&file);
        f_close(&file);
